...
class _OrdersListPageState extends ConsumerState<OrdersListPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final asyncData = ref.watch(ordersListProvider(params));
                    ...
                    child: ListView.separated(
                      ...
                      itemBuilder: (context, i) {
                        ...
                        return ListTile(
                          ...
                          title: Text(
                            'Order #${row.orderNo ?? row.id ?? 'N/A'}',
                            style: const TextStyle(fontWeight: FontWeight.w500),
                          ),
                          subtitle: Text(
                            _formatOrderSubtitle(row),
                            maxLines: 2,
  ...
  String _formatOrderSubtitle(row) {
    final parts = <String>[];
    if (row.orderStatus != null) parts.add('Status: ${row.orderStatus}');
    if (row.deliveryStatus != null) parts.add('Delivery: ${row.deliveryStatus}');
    if (row.amount != null) parts.add('Amount: \$${row.amount}');
    if (row.orderDate != null) parts.add('Date: ${row.orderDate}');
    return parts.isEmpty ? 'Order details' : parts.join(' • ');
  }
