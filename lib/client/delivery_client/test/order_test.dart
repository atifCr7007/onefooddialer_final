import 'package:test/test.dart';
import 'package:openapi/openapi.dart';

// tests for Order
void main() {
  final instance = OrderBuilder();
  // TODO add properties to the builder and call build()

  group(Order, () {
    // Order ID
    // int id
    test('to test the property `id`', () async {
      // TODO
    });

    // Order number
    // String orderNo
    test('to test the property `orderNo`', () async {
      // TODO
    });

    // Customer code
    // int customerCode
    test('to test the property `customerCode`', () async {
      // TODO
    });

    // Customer name
    // String customerName
    test('to test the property `customerName`', () async {
      // TODO
    });

    // Customer phone number
    // String customerPhone
    test('to test the property `customerPhone`', () async {
      // TODO
    });

    // Shipping address
    // String shipAddress
    test('to test the property `shipAddress`', () async {
      // TODO
    });

    // Order date
    // Date orderDate
    test('to test the property `orderDate`', () async {
      // TODO
    });

    // Delivery status
    // String deliveryStatus
    test('to test the property `deliveryStatus`', () async {
      // TODO
    });

    // Order status
    // String orderStatus
    test('to test the property `orderStatus`', () async {
      // TODO
    });

    // Delivery person ID
    // int deliveryPerson
    test('to test the property `deliveryPerson`', () async {
      // TODO
    });

    // Location code
    // int locationCode
    test('to test the property `locationCode`', () async {
      // TODO
    });

    // DeliveryLocation location
    test('to test the property `location`', () async {
      // TODO
    });

    // Order amount
    // double amount
    test('to test the property `amount`', () async {
      // TODO
    });

    // Tax amount
    // double tax
    test('to test the property `tax`', () async {
      // TODO
    });

    // Delivery charges
    // double deliveryCharges
    test('to test the property `deliveryCharges`', () async {
      // TODO
    });

    // Applied discount
    // double appliedDiscount
    test('to test the property `appliedDiscount`', () async {
      // TODO
    });

    // Total amount
    // double total
    test('to test the property `total`', () async {
      // TODO
    });

    // Payment mode
    // String paymentMode
    test('to test the property `paymentMode`', () async {
      // TODO
    });

    // Whether the amount is paid
    // bool amountPaid
    test('to test the property `amountPaid`', () async {
      // TODO
    });

    // Kitchen code
    // String fkKitchenCode
    test('to test the property `fkKitchenCode`', () async {
      // TODO
    });

    // Order menu type
    // String orderMenu
    test('to test the property `orderMenu`', () async {
      // TODO
    });

    // Delivery time
    // String deliveryTime
    test('to test the property `deliveryTime`', () async {
      // TODO
    });

    // Delivery end time
    // String deliveryEndTime
    test('to test the property `deliveryEndTime`', () async {
      // TODO
    });

    // Delivery type
    // String deliveryType
    test('to test the property `deliveryType`', () async {
      // TODO
    });

    // Third-party delivery order ID
    // String tpDeliveryOrderId
    test('to test the property `tpDeliveryOrderId`', () async {
      // TODO
    });

  });
}
